var ClearCommand, Client, Connection, ForwardCommand, FrameBufferCommand,
  GetDevicePathCommand, GetFeaturesCommand, GetPackagesCommand,
  GetPropertiesCommand, GetSerialNoCommand, GetStateCommand, HostConnectCommand,
  HostDevicesCommand, HostDevicesWithPathsCommand, HostDisconnectCommand,
  HostKillCommand, HostTrackDevicesCommand, HostTransportCommand,
  HostVersionCommand, InstallCommand, IsInstalledCommand, ListForwardsCommand,
  LocalCommand, LogCommand, Logcat, LogcatCommand, Monkey, MonkeyCommand,
  Parser, ProcStat, Promise, RebootCommand, RemountCommand, ScreencapCommand,
  ShellCommand, StartActivityCommand, StartServiceCommand, Sync, SyncCommand,
  TcpCommand, TcpIpCommand, TcpUsbServer, TrackJdwpCommand, UninstallCommand,
  UsbCommand, WaitBootCompleteCommand, WaitForDeviceCommand, debug;
...
Client = (function() {
  ...
  Client.prototype.connection = function() {
    ...
    conn = new Connection(this.options).on('error', errorListener =
      function(err) {
  ...
  Client.prototype.transport = function(serial, callback) {
    return this.connection().then(function(conn) {
      return new HostTransportCommand(conn).execute(serial)["return"]
        (conn);
    }).nodeify(callback);
  };
  ...
  Client.prototype.screencap = function(serial, callback) {
    return this.transport(serial).then((function(_this) {
      return function(transport) {
        return new ScreencapCommand(transport).execute()["catch"]
          (function(err) {
    ...
    tryConnect = (function(_this) {
      return function(times) {
        return _this.openTcp(serial, port).then(function(stream) {
          ...
          if (times -= 1) {
            debug("Monkey can't be reached, trying " + times +
              " more times");
  ...
  Client.prototype.openMemStat = function(serial, callback) {
    return this.syncService(serial).then(function(sync) {
      return new MemStat(sync);
    }).nodeify(callback);
  };
    ...
    return this.push(serial, apk, temp).then((function(_this) {
      return function(transfer) {
        ...
        transfer.on('end', endListener = function() {
          return resolver.resolve(_this.installRemote(serial,
            temp));
        });
        return resolver.promise["finally"](function() {
  ...
  Client.prototype.installRemote = function(serial, apk, callback) {
    return this.transport(serial).then((function(_this) {
      return function(transport) {
        return new InstallCommand(transport).execute(apk).then(
          function() {
