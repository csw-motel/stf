          ...
          return plugin.stop()
            ...
            .then(function(logcat) {
              ...
              function entryListener(entry) {
                push.send([
                  dev.channel, wireutil.envelope(new wire.DeviceLogcatEntryMessage(
                    options.serial, entry.date.getTime() / 1000, entry.pid, entry.tid,
                    entry.priority, entry.tag, entry.message
                  ))
                ])
              }
