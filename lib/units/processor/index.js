...
module.exports = function(options) {
  ...
  .on(wire.AddDeviceToUserMessage, function(channel, message, data) {
    log.info('MJ response: entry')
    var messageUser

    dbapi.loadAccessToken(message.token)
      .then(function(token) {
        if (!token) {
          log.info('MJ response: no token')
          ...
          dbapi.loadUser(token.email)
            .then(function(user) {
              if (user) {
                messageUser = user
                log.info('MJ response: user ' + user.name + ' email ' + user.email)
              }
              else {
                log.info('MJ response: NO user')
                appDealer.send([
                  channel, wireutil.envelope(
                    new wire.UserDeviceResponseMessage(
                      message.token, message.serial, false
          ...
          dbapi.loadDevice(message.serial)
            .then(function(device) {
              log.info('MJ response: load device')
              if (!device) {
                log.info('MJ response: NO DEVICE DATA')
                appDealer.send([
                  channel, wireutil.envelope(
                    new wire.UserDeviceResponseMessage(
                      message.token, message.serial, false
                ...
                if (!deviceutil.isAddable(device, messageUser)) {
                  log.info('MJ response: Device is being used or not available')
                  appDealer.send([
                    channel, wireutil.envelope(
                      new wire.UserDeviceResponseMessage(
                        message.token, message.serial, false
                ...
                var messageListener = wirerouter()
                  .on(wire.JoinGroupMessage, function(channel, message) {
                    if (message.serial === device.serial &&
                      ...
                      channelRouter.removeListener(wireutil.global, messageListener)
                      log.info('MJ response: Device added')
                      appDealer.send([
                        channel, wireutil.envelope(
                          new wire.UserDeviceResponseMessage(
                ...
                devDealer.send([
                  device.channel, wireutil.envelope(
                    new wire.GroupMessage(
                      new wire.OwnerMessage(
                        messageUser.email, messageUser.name, messageUser.group
            ...
            .catch(function(err) {
              log.error('Failed to load device "%s": ')
            })
        }
      })
  })
  ...
  .on(wire.DeleteDeviceFromUserMessage, function(channel, message, data) {
    log.info('MJ response delete: entry')
    var messageUser

    dbapi.loadAccessToken(message.token)
      .then(function(token) {
        if (!token) {
          log.info('MJ response delete: no token')
          channelRouter.send(wireutil.global,
            wireutil.envelope(
              new wire.UserDeviceResponseMessage(
                message.token, message.serial, false
          ...
          if (!data) {
            log.info('MJ response delete: no data')
            appDealer.send([
              channel, wireutil.envelope(
                new wire.UserDeviceResponseMessage(
                  message.token, message.serial, false
          ...
          dbapi.loadUser(token.email)
            .then(function(user) {
              if (user) {
                messageUser = user
                log.info('MJ response delete: ' + user.name + ' email ' + user.email)
              }
              else {
                log.info('MJ response delete: NO user')
                appDealer.send([
                  channel, wireutil.envelope(
                    new wire.UserDeviceResponseMessage(
                      message.token, message.serial, false
          ...
          dbapi.loadDevice(message.serial)
            .then(function(device) {
              log.info('MJ response delete: load device')
              if (!device) {
                log.info('MJ response delete: NO DEVICE DATA')
                appDealer.send([
                  channel, wireutil.envelope(
                    new wire.UserDeviceResponseMessage(
                      message.token, message.serial, false
                ...
                if (!deviceutil.isOwnedByUser(device, messageUser)) {
                  log.info('You cannot release this device. Not owned by you')
                  appDealer.send([
                    channel, wireutil.envelope(
                      new wire.UserDeviceResponseMessage(
                        message.token, message.serial, false
                ...
                var messageListener = wirerouter()
                  .on(wire.LeaveGroupMessage, function(channel, message) {
                    if (message.serial === device.serial &&
                      ...
                      clearTimeout(responseTimer)
                      channelRouter.removeListener(wireutil.global, messageListener)
                      log.info('MJ response delete:: Device deleted')
                      appDealer.send([
                        channel, wireutil.envelope(
                          new wire.UserDeviceResponseMessage(
                            message.token, message.serial, true
