  ...
  devDealer.on('message', wirerouter()
    ...
    .on(wire.DeviceIdentityMessage, function(channel, message, data) {
      ...
      dbapi.loadDevice(message.serial)
        ...
        if(message.manufacturer === 'Apple') {
         dbapi.saveDeviceStatus(message.serial, 3)
        }
    })
    ...
    .on(wire.DeviceCpuPerformanceMessage, function(channel, message, data) {
      appDealer.send([channel, data])
    })
    ...
    .on(wire.TokenAuthMessage, function(channel, message, data) {
      appDealer.send([channel, data])
    })
     .on(wire.RequestListPlugAndPlayMessage, function(){
        ...
        return Promise.promisify(cursor.toArray, cursor)()
          .then(function(list) {
            ...
            list.forEach(function(device) {
              prepareList.push(new wire.DevicePlugPlayMessage({
            ...
            devDealer.send([wireutil.global,
                wireutil.envelope(new wire.PlugPlayListMessage({list:prepareList}))])
          })
      })
    })
    .handler())
