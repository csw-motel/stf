                    ...
                    .on(wire.JoinGroupMessage, function(channel, message) {
                      if (message.serial === device.serial &&
                        ...
                        appDealer.send([
                          channel, wireutil.envelope(
                            new wire.UserDeviceResponseMessage(
                              message.token, message.serial, true, 'Device successfully added'
                            )
                          )
                        ])
                      }
                  ...
                  if (!deviceutil.isOwnedByUser(device, messageUser)) {
                    appDealer.send([
                      channel, wireutil.envelope(
                        new wire.UserDeviceResponseMessage(
                          message.token, message.serial, false,
                          'You cannot release this device. Not owned by you'
                        )
                      )
                    ])
                  }
                    ...
                    .on(wire.LeaveGroupMessage, function(channel, message) {
                      if (message.serial === device.serial &&
                        ...
                        appDealer.send([
                          channel, wireutil.envelope(
                            new wire.UserDeviceResponseMessage(
                              message.token, message.serial, true, 'Device successfully removed'
                            )
                          )
                        ])
                      }
