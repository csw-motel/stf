      ...
      dbapi.loadAccessToken(message.token)
        ...
        .catch(function(err) {
          ...
          push.send([
            channel, wireutil.envelope(
              new wire.UserDeviceResponseMessage(
                message.token, message.serial, false, 'Failed to load token: '
              )
            )
          ])
        })
      ...
      dbapi.loadAccessToken(message.token)
        .then(function(token) {
          if (!token) {
            push.send([channel
              , wireutil.envelope(
                new wire.AuthResponseMessage(
                  false, message.token, 'Token not found', null
                )
              )
            ])
          }
            ...
            dbapi.loadUser(token.email)
              .then(function(user) {
                if (user) {
                  push.send([
                    channel, wireutil.envelope(
                      new wire.AuthResponseMessage(
                       true, message.token, 'Token successfully authorized', user.email
                      )
                    )
                  ])
                }
                  ...
                  push.send([
                    channel, wireutil.envelope(
                      new wire.AuthResponseMessage(
                       false, message.token, 'This user do not exist', null
                      )
                    )
                  ])
                }
              ...
              .catch(function(err) {
                ...
                push.send([
                  channel, wireutil.envelope(
                    new wire.AuthResponseMessage(
                      false, message.token, 'Failed to load user: ', null
                    )
                  )
                ])
              })
        ...
        .catch(function(err) {
          ...
          push.send([
            channel, wireutil.envelope(
              new wire.AuthResponseMessage(
                false, message.token, 'Failed to load token: ', null
              )
            )
          ])
        })
